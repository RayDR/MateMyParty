...
Token candidate queries join the guest row and require both `invitations.revoked_at IS NULL` and `guests.archived_at IS NULL`. Public detail resolution repeats those predicates inside the tracking transaction. This is the lookup boundary for the next milestone: future lookup can return active invitation summaries, but it cannot reconstruct or return a permanent raw token. Restoring an archived guest makes its still-active historical invitation eligible again; regeneration remains the explicit way to issue a different credential.
