    ...
    <>
      {panelOpen ? (
        <section
          ...
          className={`${preview ? 'absolute' : 'fixed'} invitation-rsvp-panel inset-x-3 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-50 mx-auto max-h-[min(68dvh,42rem)] w-auto min-w-0 max-w-2xl overflow-x-hidden overflow-y-auto rounded-3xl border border-cyan-300/20 bg-slate-950/96 p-5 shadow-2xl backdrop-blur-xl @md:p-7`}
        >
      ...
      <footer
        ...
        className={`${preview ? 'absolute' : 'fixed'} invitation-rsvp-footer inset-x-0 bottom-0 z-40 w-full max-w-[100dvw] overflow-x-clip border-t border-white/15 bg-slate-950/96 px-2 pt-2 pb-[calc(.45rem+env(safe-area-inset-bottom))] shadow-[0_-0.5rem_1.5rem_rgb(2_6_23_/_24%)] backdrop-blur-xl`}
      >
